...
const IncidentQuestions = ({
    ...
    const handleAddTag = (id: string) => {
        ...
        const existing = (questions.find(q => q.id === id)?.options || []).concat(customTags);
    ...
    const renderInput = (q: Question) => {
        ...
        switch (q.type) {
                ...
                case "radio":
                    ...
                    <div className="flex flex-wrap gap-3 mt-2">
                        {q.options?.map(opt => {
                            ...
                            const selected = answer === opt;
                            return (
                                <label key={opt} className={`px-4 py-2 rounded-lg text-lg items-center border cursor-pointer transition ${disabled ? "bg-gray-100 text-gray-400 cursor-not-allowed" : selected ? "bg-red-100 border-red-500 text-red-700" : "bg-gray-50 border-gray-300 hover:bg-gray-100"}`}>
                                    <input
                                        type="radio"
                                        name={q.id}
                                        value={opt}
                            ...
                            {allTags.map(tag => {
                                ...
                                return (
                                    <button
                                        ...
                                        className={`px-4 py-2 rounded-full border text-lg transition ${selected ? "bg-blue-100 text-blue-700 border-blue-500" : "bg-gray-50 border-gray-300 hover:bg-gray-100"}`}
                                    >
                                        #{tag}
                                    </button>
                                );
            ...
            case "file":
                ...
                const fileTooLarge = firstFile && firstFile.size > maxSizeMB * 1024 * 1024;
                ...
                return (
                    <div className="space-y-4 mt-2 text-lg">
                        ...
                        >
                            ...
                            <div className="flex flex-col items-center justify-center gap-3 p-6 rounded-xl border border-dashed border-gray-300 bg-white shadow-sm relative">
                                ...
                                <p className="text-gray-500">
                                    Drag & drop ou <span className="text-blue-600 underline cursor-pointer">choisir un fichier</span>
                                </p>
                            ...
                            <div className="flex items-center justify-between bg-blue-50 border border-blue-200 rounded-lg px-4 py-3">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-base text-gray-500">{(firstFile.size / (1024 * 1024)).toFixed(2)} MB</p>
                                    </div>
                                </div>
                                <div className="flex items-center gap-3">
                                    <button
                        ...
                        {fileTooLarge && (
                            <p className="text-red-600 text-lg font-medium">⚠️ Le fichier dépasse la taille maximale autorisée de {maxSizeMB}MB.</p>
                        )}
                    </div>
                );
            default:
        ...
        <div className="space-y-6 text-lg">
            {paginatedQuestions.map((q) => (
                <div
                    ...
                    className={`bg-white p-6 rounded-xl shadow border ${errorFields.includes(q.id) ? "border-red-500 animate-shake" : "border-gray-200"}`}
                >
